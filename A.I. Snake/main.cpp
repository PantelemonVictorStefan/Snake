...
{   if(snake2[0].x==25*length)
    ...
    if ((snake2[0].x==fruit.x) && (snake2[0].y==fruit.y))
    {
        ...
        do
        {
            ...
            for(int i=1; i<num2; i++)
                if((snake2[i].x==fruit.x)&&(snake2[i].y)==(fruit.y))
                    valid=false;
        }
        while(valid==false);
        ...
        if(score>=10)
        {
            ...
            do
            {
                ...
                for(int i=1; i<num2; i++)
                    if((snake2[i].x==booster.x)&&(snake2[i].y)==(booster.y))
                        valid=false;
            }
            while(valid==false);
    ...
    if (snake2[0].x>=rows) gameover=true;
    if (snake2[0].x<0) gameover=true;
    if (snake2[0].y>columns-1) gameover=true;
    if (snake2[0].y<0) gameover=true;

    /*if((snake2[0].y==0) and dir2==3) dir2=2;
    if((snake2[0].x==rows-1) and dir2==2) dir2=0;
    if((snake2[0].y==columns-1) and dir2==0) dir2=1;
    if((snake2[0].x==0) and dir2==1) dir2=3;*/

    for (int i=1;i<num2;i++)
     if (snake2[0].x==snake2[i].x && snake2[0].y==snake2[i].y) gameover=true;
imput=false;
 }
 ...
 void Play()
{
    ...
    RenderWindow window(VideoMode(width, height), "snake2 Game!");
...
if(track==1)
    {
		...
		if ((Keyboard::isKeyPressed(Keyboard::Left) and dir2!=2)and imput==false) {dir2=1;imput=true;}
	    if ((Keyboard::isKeyPressed(Keyboard::Right) and dir2!=1)and imput==false)  {dir2=2;imput=true;}
	    if ((Keyboard::isKeyPressed(Keyboard::Up) and dir2!=0)and imput==false) {dir2=3;imput=true;}
		if ((Keyboard::isKeyPressed(Keyboard::Down) and dir2!=3)and imput==false) {dir2=0;imput=true;}
    ...
    //set snake2
        ...
        if(ChangeSkin)
        {
            if(TextureSkin==0)
                for (int i=0; i<num2; i++)
                {
                    sprite15.setPosition(snake2[i].x*length, snake2[i].y*length);
            ...
            if(TextureSkin==1)
                for (int i=0; i<num2; i++)
                {
                    sprite16.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite16);
                }
            if(TextureSkin==2)
                for (int i=0; i<num2; i++)
                {
                    sprite17.setPosition(snake2[i].x*length, snake2[i].y*length);
            ...
            if(TextureSkin==4)
                for (int i=0; i<num2; i++)
                {
                    sprite18.setPosition(snake2[i].x*length, snake2[i].y*length);
            ...
            if(TextureSkin==5)
                for (int i=0; i<num2; i++)
                {
                    sprite19.setPosition(snake2[i].x*length, snake2[i].y*length);
            ...
            {   window.draw(sprite32);
                if(highColor%7==0)
                     for (int i=0; i<num2; i++)
                {
                    sprite24.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite24);
                }
                if(highColor%7==1)
                 for (int i=0; i<num2; i++)
                {
                    sprite25.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite25);
                }
                if(highColor%7==2)
                 for (int i=0; i<num2; i++)
                {
                    sprite26.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite26);
                }
                if(highColor%7==3)
                 for (int i=0; i<num2; i++)
                {
                    sprite27.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite27);
                }
                if(highColor%7==4)
                 for (int i=0; i<num2; i++)
                {
                    sprite28.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite28);
                }
                if(highColor%7==5)
                 for (int i=0; i<num2; i++)
                {
                    sprite29.setPosition(snake2[i].x*length, snake2[i].y*length);
                    window.draw(sprite29);
                }
                if(highColor%7==6)
                 for (int i=0; i<num2; i++)
                {
                    sprite30.setPosition(snake2[i].x*length, snake2[i].y*length);
...
music.openFromFile("sound/menu.ogg");
    ...
    start:
    sf::RenderWindow window(sf::VideoMode(900, 400), "snake2 Game.");
